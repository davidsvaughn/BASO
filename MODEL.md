...
Standard GP regression would be suitable only for modeling the learning curve for a single benchmark. In this situation, starting with some initial set of checkpoint numbers, or indices, X = [x_1, x_2, ..., x_n] for which we have already run the benchmark to obtain function values f=[f(x_1),…,f(x_n))].⊤ we would define a GP prior over those values as a multivariate normal: 
f ~ N(m,K) where m(x) is a mean function and K is a pair-wise kernel function k(x,x') used to express the correlation between function values f(x) and f(x'). Since the input domain (model benchmarks) is numeric (as opposed to categorical) we would use an RBF kernel which represents similarities between input pairs $x,x{\prime}$ as a function of the squared distance between them $|x-x{\prime}|^2$, which encodes the intuition that model checkpoints that are closer together are expected to have more similar function values than two checkpoints that are farther apart.
