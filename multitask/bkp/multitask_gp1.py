...
# X_sampled = []  # Will hold (checkpoint_num, test_idx) pairs
# y_sampled = []  # Will hold corresponding test results
...
#-------------------------------------------------------------------------
# Xtest_list = np.array([x_star for _ in range(Z)])
# output_index = np.array([np.ones_like(x_star)*i for i in range(Z)]).reshape(-1,1)
...
# mu_list, var_list = m.predict(Xtest_list)#, Y_metadata=Y_metadata)
