...
class AdaptiveSchoolTestingModel:
    ...
    def __init__(self, num_grades=12, initial_uncertainty=0.5, min_confidence=0.95, initial_correlation=0.5):
        ...
        # dsv
        self.prior_correlation_matrix = self.correlation_matrix.copy()
        self.prior_cov = self.cov.copy()
        self.prior_grade_means = self.grade_means.copy()
        self.prior_grade_stds = self.grade_stds.copy()
        ...
        # next_school_data['sampled_grades']
    def get_next_grade_to_sample(self, school_data):
        """
        ...
        -----------
        school_data : dict
            Data for the school including sampled grades and scores
        ...
        """
        already_sampled_grades = school_data['sampled_grades']
        ...
        for g in remaining_grades:
            ...
            # weight vector for the priorities
            w = np.array([25.0, 30.0, 20.0, 15.0, 30.0])
            
            # Combined priority score - weights can be adjusted based on importance
            w /= np.sum(w)
            priority = (
                w[0] * sample_priority + 
                w[1] * uncertainty_priority + 
                w[2] * correlation_priority +
                w[3] * school_pattern_priority +
                w[4] * random_component
            )
            priorities.append((g, priority))
        
        #---------------------------------------------------
        # # NEW sampling strategy
        p = np.abs(np.array([x[1] for x in priorities]))
        
        # p = p**0.5
        
        p /= np.sum(p)
        next_grade = np.random.choice(remaining_grades, p=p)
        return next_grade
        
        #---------------------------------------------------
        # OLD :GREEDY - Select the grade with highest priority
    ...
    def add_grade_result(self, school_id, grade_index, score):
        ...
        #-----------------------------------------------------------------------------
        # update self.X_mask to indicate that this grade has been sampled
        idx = school_data['idx']
        self.X[idx, grade_index] = score # 0=> False => Don't mask out this value
        #-----------------------------------------------------------------------------
    ...
    def _update_distributions(self, grade_index, new_score, school_data):
        ...
        #-------------------------------------------------------------------------------------
        X_masked = np.ma.masked_equal(self.X, -100)
        
        col_means = np.ma.mean(X_masked, axis=0)
        col_stds = np.ma.std(X_masked, axis=0)
        
        # if n > 0 and col_stds[grade_index] is not np.ma.masked:
        #     self.grade_stds[grade_index] = (col_stds[grade_index]*n * self.prior_grade_stds[grade_index]) / (n+1)
        # else:
        #     self.grade_stds[grade_index] = self.initial_uncertainty * abs(new_score)
        
        # Compute covariance and correlation matrices
        cov_matrix = np.ma.cov(X_masked, rowvar=False)
        corr_matrix = np.ma.corrcoef(X_masked, rowvar=False)
        ...
        for j in school_data['sampled_grades']:
            ...
            n = self.grade_pair_sample_counts[grade_index, j]
            
            # check if cov_matrix[i, j] is masked
            if cov_matrix[i, j] is np.ma.masked:
                continue
            # update cov matrix
            self.cov[i, j] = (cov_matrix[i, j]*n + self.prior_cov[i, j])/ (n+1)
            self.cov[j, i] = self.cov[i, j]
            # update correlation matrix
            self.correlation_matrix[i, j] = self.cov[i, j] / (self.grade_stds[i] * self.grade_stds[j])
            self.correlation_matrix[j, i] = self.correlation_matrix[i, j]
            
            # # OR
            # if corr_matrix[i, j] is np.ma.masked:
            #     continue
            # self.correlation_matrix[i, j] = (corr_matrix[i, j]*n + self.prior_correlation_matrix[i, j])/ (n+1)
            # self.correlation_matrix[j, i] = self.correlation_matrix[i, j]
            
        #-------------------------------------------------------------------------------------
            
    
    def _update_distributions_OLD(self, grade_index, new_score, school_data):
        """
        Update distribution estimates for a specific grade using online update formulas.
        """
        # Increment sample count
        ...
        for j in school_data['sampled_grades']:
            ...
            #----------------------------------------------------------
            # compare with true_cov
            # ii,jj = 0,22
            # if (i,j) == (ii,jj) or (i,j) == (jj,ii):
            #     print(f'cov({i},{j}): {self.true_cov[i,j]:0.4f}\t{self.cov[i,j]:0.4f}\t{self.grade_pair_sample_counts[i,j]}')
    ...
    def print_error(self):
        ...
        fro_norm = np.linalg.norm(self.true_corr - self.correlation_matrix, 'fro')
...
def demonstrate_top_k_rankings():
    ...
    model.true_X = X
    model.X = np.ones_like(X) * -100 # typical mask value
    
    # compute true correlation and covariance
    true_correlation_matrix = np.corrcoef(X, rowvar=False)
    true_covariance_matrix = np.cov(X, rowvar=False)
    true_means = np.mean(X, axis=0)
    true_stds = np.std(X, axis=0)
    model.true_corr = true_correlation_matrix
    model.true_cov = true_covariance_matrix
    ...
    #--------------------------------------------------------------------------------
    # METHOD 1: test same N random grades for every school
    ...
    # for i in school_indices:
    #     school = true_scores[i]
    #     for grade_index in grade_indices[:2]:
    #         score = school['grade_scores'][grade_index]
    #         model.add_grade_result(school['school_id'], grade_index, score, update_rankings=False)
    #         tests_conducted += 1
    #--------------------------------------------------------------------------------
    ...
    while tests_conducted < max_tests:
        ...
        if prob_best.sum() == 0:
            ...
            # get argmax of prob_best
            # if np.random.rand() < 0.5:
            #     next_school_idx = np.argmax(prob_best)
        #-----------------------------------------------------------
        ...
        next_grade = model.get_next_grade_to_sample(next_school_data)
        ...
        if tests_conducted % check == 0:
            ...
            # print(model.grade_pair_sample_counts)
