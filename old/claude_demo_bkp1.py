...
def demonstrate_top_k_rankings():
    ...
    K = 10      # top K
    N = 0.15    # initial grades to test for each school
    check = 25  # check every N tests

    # initial_uncertainty = 5.0 # initial uncertainty in grade scores (~ 10x std dev)
    # initial_correlation = 0.5  # initial correlation between grades
    
    # initial_uncertainty, initial_correlation = 5.0, 0.5 
    # initial_uncertainty, initial_correlation = 0.1, 0.1
    initial_uncertainty, initial_correlation = 1.0, 0.25
    ...
    true_scores, X = load_school_data(f'./data/{fn}')
    ...
    while tests_conducted < max_tests:
        ...
        if tests_conducted % check == 0:
            top_k = get_top_k_schools(model, k=K)
            top_k_history[tests_conducted] = top_k
            print(f"\nAfter {tests_conducted}/{tot_tests} tests:")
            print_top_k(top_k, true_rankings)
    ...
    # compute the true correlation matrix from X
    true_correlation = np.corrcoef(X, rowvar=False)
    cdiff = np.abs(model.correlation_matrix - true_correlation)
