...
class GPWithEmbeddings:
    ...
    def __init__(self, n_tests, embedding_dim=5, random_seed=42):
        ...
        self.test_embeddings = torch.nn.Parameter(
            torch.randn(n_tests, embedding_dim) * 0.1
        )
        
        # Setup the kernel and GP model
        self.kernel = ConstantKernel(1.0) * RBF(length_scale=1.0) + WhiteKernel(0.01)
        ...
        self.optimizer = torch.optim.Adam([self.test_embeddings], lr=0.01)
    ...
    def encode_features(self, checkpoint_nums, test_indices):
        ...
        # Stack with checkpoint numbers
        checkpoint_tensor = torch.tensor(checkpoint_nums, dtype=torch.float32).unsqueeze(1)
        features = torch.cat([checkpoint_tensor, batch_embeddings], dim=1)
        
        return features
    
    def negative_log_likelihood(self, X_checkpoint, X_test_idx, y):
        """Calculate negative log likelihood for GP with current embeddings"""
        # Create feature vectors with current embeddings
        X_encoded = self.encode_features(X_checkpoint, X_test_idx)
        X_np = X_encoded.detach().numpy()
        
        # Fit GP with these features
        self.gp.fit(X_np, y)
    ...
    def optimize_embeddings(self, X_checkpoint, X_test_idx, y, n_iter=100):
        ...
        X_checkpoint_tensor = torch.tensor(X_checkpoint, dtype=torch.float32)
        X_test_idx_tensor = torch.tensor(X_test_idx, dtype=torch.long)
        ...
        for i in range(n_iter):
            ...
            loss = self.negative_log_likelihood(
                X_checkpoint_tensor, X_test_idx_tensor, y
            )
            losses.append(loss)
        ...
        self.gp.fit(X_encoded.detach().numpy(), y)
    ...
    def predict(self, X_checkpoint, X_test_idx, return_std=False):
        ...
        # Predict with GP
        return self.gp.predict(X_encoded.detach().numpy(), return_std=return_std)
...
class DiverseAdaptiveSampler:
    ...
    def __init__(self, data_file, test_subset=None, random_seed=42, embedding_dim=5):
        ...
        self.test_embedding_dim = embedding_dim
        self.test_embeddings = np.random.normal(0, 0.1, size=(self.n_tests, embedding_dim))
    ...
    def configure_kernel(self):
        """Configure a single anisotropic kernel with correct dimensionality."""
        # Total input dimensionality is 1 (checkpoint) + test_embedding_dim
        total_dims = 1 + self.test_embedding_dim
        
        # Initialize all length scales with default value
        length_scales = np.ones(total_dims)
        
        # Optional: Set different initial length scales for checkpoint vs embedding
        # length_scales[0] = 10.0  # Different initial scale for checkpoint dimension
        
        # Create kernel with correct dimensionality
    ...
    #---------------------------------------------------------------------------------------------------
    # def configure_kernel(self):
    #     """Configure a kernel appropriate for mixed embeddings."""
    #     # Separate kernels for numerical and embedding parts
    #     checkpoint_kernel = RBF(length_scale=1.0, length_scale_bounds=(1e-2, 1e2))
        
    #     # For test embeddings, use a kernel with separate length scales per dimension
    #     # This allows the GP to learn which embedding dimensions are most important
    #     test_kernel = RBF(
    #         length_scale=np.ones(self.test_embedding_dim),
    ...
    def update_model(self):
        """Update GP model and optimize test embeddings."""
        # Skip if too few samples
        if len(self.y_sampled) < 5:
            return None
        
        # Initial optimization of GP with current embeddings
        X = np.array(self.X_sampled)
        y = np.array(self.y_sampled)
        X_encoded = self._encode_features(X)
        self.gp.fit(X_encoded, y)
        
        # Optimize embeddings with gradient descent
        # This requires modifying the GP to allow gradient updates to the embeddings
        # Here's a simplified approach
        learning_rate = 0.01
        for _ in range(10):  # 10 gradient steps
            # For each test with samples
            for test_idx in range(self.n_tests):
                # Find samples for this test
                test_samples = [i for i, x in enumerate(self.X_sampled) if x[1] == test_idx]
                if not test_samples:
                    continue
                    
                # Get predictions with current embeddings
                X_test = np.array([self.X_sampled[i] for i in test_samples])
                y_test = np.array([self.y_sampled[i] for i in test_samples])
                X_encoded = self._encode_features(X_test)
                
                # Compute gradient and update (simplified approximation)
                y_pred = self.gp.predict(X_encoded)
                error = y_pred - y_test
                # print(f"Error = {np.sum(error)}")
                
                # Update embedding based on prediction error (simplified)
                for dim in range(self.test_embedding_dim):
                    gradient = np.sum(error) * learning_rate
                    self.test_embeddings[test_idx, dim] -= gradient
        
        # Final update of GP with optimized embeddings
        X_encoded = self._encode_features(X)
        self.gp.fit(X_encoded, y)
        return self.gp
