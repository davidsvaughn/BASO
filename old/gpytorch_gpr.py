...
class ExactGPModel(gpytorch.models.ExactGP):
    """
    Custom GPyTorch model for Gaussian Process regression.
    """
    def __init__(self, train_x, train_y, likelihood):
        ...
        # Covariance module - RBF kernel + constant kernel + white noise
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.RBFKernel(ard_num_dims=input_dim)
        ) #+ gpytorch.kernels.white_noise.WhiteNoiseKernel()
...
class DiverseAdaptiveSampler:
    ...
    estimate peak checkpoint performance using GPyTorch-accelerated Gaussian Process Regression.
    """
    ...
    def __init__(self, data_file, random_seed=42, use_gpu=True):
        ...
        self.model = None
        self.training_iterations = 100  # Default training iterations
    ...
    def update_model(self, training_iterations=None):
        """Update the GPyTorch model with current samples"""
        ...
        try:
            ...
            X_tensor = torch.tensor(X_encoded, dtype=torch.float32, device=self.device)
    ...
    def predict_performance(self, checkpoint_nums=None):
        ...
        if self.model is None or self.likelihood is None:
            print("Model not yet trained. Running update_model first.")
            self.update_model()
            
            # If still not available, return empty dataframe
            if self.model is None:
                return pd.DataFrame(columns=['checkpoint_num', 'predicted_value', 'uncertainty'])
    ...
    def acquisition_function(self, strategy='ucb', kappa=2.0):
        ...
        try:
            # Convert to PyTorch tensor
            X_tensor = torch.tensor(X_unsampled_encoded, dtype=torch.float32, device=self.device)
            
            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                # Get predictions
                predictions = self.likelihood(self.model(X_tensor))
                mu = predictions.mean.cpu().numpy()
                variance = predictions.variance.cpu().numpy()
    ...
    def run_sampling(self, n_samples, acquisition_strategy='ucb', kappa=2.0,
                   ...
                   force_exploration_every=5, training_iterations=100, verbose=True):
        """
        ...
        self.update_model(training_iterations=training_iterations)
...
if __name__ == "__main__":
    ...
    sampler = DiverseAdaptiveSampler(
        "data/phi4-math-4claude.txt",
        random_seed=rand_seed,
        use_gpu=torch.cuda.is_available()  # Use GPU if available
    )
    
    # Run sampling with GPU acceleration
    predictions = sampler.run_sampling(
        ...
        diversity_weight=0.5,    # Balance between utility and diversity
        force_exploration_every=10,  # Force exploration frequently
        training_iterations=100,  # Faster convergence with GPyTorch
        verbose=True
    )
