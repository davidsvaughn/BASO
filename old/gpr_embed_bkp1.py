...
class GPWithEmbeddings:
    ...
    def __init__(self, n_tests, embedding_dim=3, random_seed=42, device=None):
        ...
        self.output_scale = torch.nn.Parameter(torch.tensor(1.0, device=self.device))
        self.length_scale = torch.nn.Parameter(torch.tensor(0.1, device=self.device))
        self.noise = torch.nn.Parameter(torch.tensor(0.01, device=self.device))
    ...
    def negative_log_likelihood(self, X_encoded, y_tensor):
        ...
        # # Preserve gradient flow by checking tensor type
        # if isinstance(y, torch.Tensor):
        #     y_tensor = y.to(self.device).float()
        # else:
    ...
    def optimize_embeddings(self, X_checkpoint, X_test_idx, y, n_iter=100):
        ...
        # y_tensor = torch.tensor(y, dtype=torch.float32, device=self.device) if not isinstance(y, torch.Tensor) else y
        # Preserve gradient flow by checking tensor type
