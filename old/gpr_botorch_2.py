...
class DiverseAdaptiveSampler:
    ...
    def initialize_sampling(self, n_initial=10, strategy='grid'):
        ...
        if strategy == 'random':
            ...
            bounds[0, 1] = 1.0  # Upper bound for first dimension
            bounds[1, 1] = 1.0  # Upper bound for second dimension
    ...
    def update_model(self):
        ...
        # Fit model using training routine
        self.fit_model()
    ...
    def fit_model(self):
        """Fit the GP model using custom training."""
        ...
        # Train with settings for better performance
    ...
    def predict_performance(self, checkpoint_nums=None):
        """Predict mean performance for each checkpoint across all tests using BoTorch with optimizations."""
        ...
        try:
            ...
            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                ...
                for i in range(0, len(X_tensor), batch_size):
                    ...
                    mu_batch = posterior.mean.cpu().numpy()
                    var_batch = posterior.variance.cpu().numpy()
                    sigma_batch = np.sqrt(var_batch)
                    
                    # Denormalize predictions
                    mu_batch = mu_batch * self.y_std.cpu().numpy() + self.y_mean.cpu().numpy()
                    sigma_batch = sigma_batch * self.y_std.cpu().numpy()
